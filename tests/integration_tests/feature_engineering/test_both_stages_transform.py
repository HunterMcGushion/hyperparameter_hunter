...
normal_holdout_inputs = [[boston_head_inputs.iloc[[3], :]] * 3] * 2
normal_holdout_targets = [[boston_head_targets.iloc[[3], :]] * 3] * 2
...
def test_all_wrangled_targets(boston_head_env_fixture, intra_cv_experiment_fixture, end_data):
    ...
    agg_datasets["on_fold_end"] = chunked(agg_datasets["on_fold_end"], 3)  # Number of folds

    for r, rep_datasets in enumerate(agg_datasets["on_fold_end"]):
        for f, datasets in enumerate(rep_datasets):
            assert_array_almost_equal(datasets["fold_train_input"], end_data[0][r][f])
            assert_array_almost_equal(datasets["fold_train_target"], end_data[1][r][f])
            assert_array_almost_equal(datasets["fold_validation_input"], end_data[2][r][f])
            assert_array_almost_equal(datasets["fold_validation_target"], end_data[3][r][f])
            assert_array_almost_equal(datasets["fold_holdout_input"], end_data[4][r][f])
            assert_array_almost_equal(datasets["fold_holdout_target"], end_data[5][r][f])
