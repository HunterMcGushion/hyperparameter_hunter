...
class LoggerFitStatus(BaseLoggerCallback):
    ...
    def on_repetition_start(self):
        G.log('Starting Repetition {}'.format(self._rep), previous_frame=inspect.currentframe().f_back)
    ...
    def on_fold_end(self):
        ...
        content += 'Time Elapsed: {}'.format(sec_to_hms(self.stat_aggregates['times']['folds'][-1], as_str=True))
...
class LoggerOOF(BaseLoggerCallback): pass


class LoggerHoldout(BaseLoggerCallback): pass


class LoggerTest(BaseLoggerCallback): pass
