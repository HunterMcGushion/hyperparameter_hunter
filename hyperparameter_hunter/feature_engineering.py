...
class PreprocessingPipelineMixIn(object):
    def __init__(
            self, pipeline, preprocessing_params,
            features, target_column,

            train_input_data: object=None, train_target_data: object=None,
            validation_input_data: object=None, validation_target_data: object=None,
            holdout_input_data: object=None, holdout_target_data: object=None,
        ...
        self.validation_input_data = validation_input_data.copy() if validation_input_data is not None else None
        self.validation_target_data = validation_target_data.copy() if validation_target_data is not None else None
        self.holdout_input_data = holdout_input_data.copy() if holdout_input_data is not None else None
        self.holdout_target_data = holdout_target_data.copy() if holdout_target_data is not None else None
        ...
        self.all_input_sets = self.get_non_null(['{}_input_data'.format(_) for _ in ('train', 'validation', 'holdout', 'test')])
        self.all_target_sets = self.get_non_null(['{}_target_data'.format(_) for _ in ('train', 'validation', 'holdout')])
    ...
    def initialize_preprocessing_stage(self):
        ...
        if self.preprocessing_stage in _stages:
            ...
            if all([_ for _ in _i_sets]):
                ...
                _err = "Inference failed. {} types must be same. Received: {}".format(_i_strs, [type(_) for _ in _i_sets])
            else:
                return 'pre_cv'
        else:
            _err = "preprocessing_stage must be in {}. Received type {}: {}".format(_stages, *type_val(self.preprocessing_stage))
    ...
    def build_pipeline(self):
        ...
        if not isinstance(self.pipeline, list):
            raise TypeError('Expected pipeline of type list. Received {}: {}'.format(*type_val(self.pipeline)))
        ...
        for i, step in enumerate(self.pipeline):
            ...
            if isinstance(step, str):
                ...
                if step_callable is None:
                    raise AttributeError('Expected pipeline value to name a method. Received {}: {}'.format(*type_val(step)))
            ##################################################
            ...
            ##################################################
                ...
                raise TypeError('Expected pipeline step to be: a str, or a tuple pair. Received {}: {}'.format(*type_val(step)))

            ##################################################
            # Additional Error Handling
            ##################################################
            if step_id is None:
                raise TypeError('Expected str as first value in each pipeline tuple. Received {}: {}'.format(*type_val(step[0])))
    ...
    def custom_pipeline_method_builder(self, functionality, name=None):
        ...
        if not callable(functionality):
            raise TypeError('Custom pipeline methods must be callable. Received type {}'.format(type(functionality)))
        ...
        while hasattr(self, name):
            ...
            G.warn('Encountered naming conflict in custom_pipeline_method_builder with "{}". Trying "{}"'.format(name, _name))
            name = _name
    ...
    def data_imputation(self, which_sets=None):
        ...
        for data_key in which_sets:
            ...
            if data is not None:
                ...
                elif any([isinstance(imputer, _) for _ in (int, float)]):  # Fill Null Data With Given Value
    ...
    def data_scaling(self, which_sets=None):
        ...
        if self.preprocessing_params.get('apply_standard_scale', False) is True:
            ...
            if 'train_input_data' in self.all_input_sets:
                self.train_input_data[self.features] = scaler.transform(self.train_input_data[self.features].values)
            if 'holdout_input_data' in self.all_input_sets:
                self.holdout_input_data[self.features] = scaler.transform(self.holdout_input_data[self.features].values)
            if 'test_input_data' in self.all_input_sets:
                self.test_input_data[self.features] = scaler.transform(self.test_input_data[self.features].values)

        G.log('Completed data_scaling preprocessing. preprocessing_params["apply_standard_scale"]={}'.format(
...
class PreCVPreprocessingPipeline(PreprocessingPipelineMixIn):
    def __init__(
            ...
            holdout_input_data, holdout_target_data,
            test_input_data=None
    ):
        PreprocessingPipelineMixIn.__init__(
            self, preprocessing_stage='pre_cv', pipeline=None, fitting_guide=None,
            preprocessing_params=None, features=features, target_column=target_column,
            train_input_data=train_input_data, train_target_data=train_target_data,
            validation_input_data=None, validation_target_data=None,
            holdout_input_data=holdout_input_data, holdout_target_data=holdout_target_data,
...
class IntraCVPreprocessingPipeline(PreprocessingPipelineMixIn):
    def __init__(
            self, features, target_column,
            train_input_data, train_target_data,
            validation_input_data, validation_target_data,
            holdout_input_data=None, holdout_target_data=None,
            test_input_data=None
    ):
        PreprocessingPipelineMixIn.__init__(
            self, preprocessing_stage='intra_cv', pipeline=None, fitting_guide=None,
            preprocessing_params=None, features=features, target_column=target_column,
            train_input_data=train_input_data, train_target_data=train_target_data,
            validation_input_data=validation_input_data, validation_target_data=validation_target_data,
            holdout_input_data=holdout_input_data, holdout_target_data=holdout_target_data,
            test_input_data=test_input_data,
            fail_gracefully=False
        )
...
class Sampler():
    ...
    def advance_pipeline(self, element):
        ...
        if element['method'] == 'smote':
            self.input_data, self.target_data = SMOTE(**element['parameters']).fit_sample(self.input_data, self.target_data)
        elif element['method'] == 'adasyn':
            self.input_data, self.target_data = ADASYN(**element['parameters']).fit_sample(self.input_data, self.target_data)
        elif element['method'] == 'RandomOverSampler':
            self.input_data, self.target_data = RandomOverSampler(**element['parameters']).fit_sample(
        ...
        # FLAG: imblearn functions return ndarray when give pandas series - FIGURE THAT SHIT OUT
            ...
            self.input_data, self.target_data = upsample(
                self.input_data, self.target_data, element['target_feature'], element['target_value'],
                **element['parameters']
            )
    ...
    def report_status(self, method=None):
        if method is None:
            ...
            print('After Performing... {}... Target Label Counts... {}'.format(
