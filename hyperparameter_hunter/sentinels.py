...
class Sentinel(metaclass=ABCMeta):
    def __init__(self, *args, **kwargs):
        """Base class for Sentinels representing data that is not yet available. Subclasses should call `super().__init__()` at
        the end of their `__init__` methods
    ...
    def sentinel(self, value: str):
        """Set :attr:`Sentinel._sentinel` to `value`, then add this instance to `settings.G.sentinel_registry`
...
class DatasetSentinel(Sentinel):
    def __init__(self, dataset_type, dataset_hash, cross_validation_type=None, global_random_seed=None, random_seeds=None):
        ...
        ----------
        dataset_type: Str
            String specifying the dataset type, suffixed with '_input', or '_target', for which a sentinel should be created.
            Acceptable values are as follows: ['train_input', 'train_target', 'validation_input', 'validation_target',
            'holdout_input', 'holdout_target']
        dataset_hash: Str
        ...
        cross_validation_type: Str, or None, default=None
            If None, `dataset_type` should be one of ['holdout_input', 'holdout_target']. Else, should be a string that is
            one of the following: 1) a string attribute of `sklearn.model_selection._split`, or 2) a hash produced while creating
            :attr:`hyperparameter_hunter.environment.Environment.cross_experiment_key`
        global_random_seed: Int, or None, default=None
            If int, should be :attr:`hyperparameter_hunter.environment.Environment.global_random_seed`. If None, `dataset_type`
            should be one of ['holdout_input', 'holdout_target']
        random_seeds: List, or None, default=None
            If list, should be :attr:`hyperparameter_hunter.environment.Environment.random_seeds`. If None, `dataset_type` should
            be one of ['holdout_input', 'holdout_target']"""
